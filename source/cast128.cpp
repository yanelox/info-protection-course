...
uint32_t get(uint32_t x[4], uint32_t n) {
	uint32_t byte_n = n / 4;
	uint32_t shift = (3 - n % 4) * 8;
	return (x[byte_n] >> shift) & 0xFF;
}

void split(uint32_t I, uint8_t &Ia, uint8_t &Ib, uint8_t &Ic, uint8_t &Id) {
	Ia = (I >> 24) & 0xFF;
	Ib = (I >> 16) & 0xFF;
	Ic = (I >> 8) & 0xFF;
	Id = I & 0xFF;
...
uint32_t cyclic_left_shift(uint32_t x, uint32_t shift) {
...
uint64_t cast128::decrypt(uint64_t msg, uint64_t key[2]) {
	return cast128::process(msg, key, false);
}
